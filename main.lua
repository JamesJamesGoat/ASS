...
local library = loadstring(game:HttpGet("https://raw.githubusercontent.com/JamesJamesGoat/ASS/main/library.lua"))()
getgenv().api = loadstring(game:HttpGet("https://raw.githubusercontent.com/JamesJamesGoat/ASS/main/api.lua"))()
local bssapi = loadstring(game:HttpGet("https://raw.githubusercontent.com/JamesJamesGoat/ASS/main/bssapi.lua"))()
...
for _, v in pairs(game:GetService("CoreGui"):GetDescendants()) do
    if v:IsA("TextLabel") and v.Text:find("macrov2 v") then
...
commandopad.Position = Vector3.new(534.851196, 61.6348724, 165.767883)

-- mondopad

local mondopad = Instance.new("Part", game.Workspace)
...
function farm(trying)
    if macrov2.toggles.loopfarmspeed then
        player.Character.Humanoid.WalkSpeed = macrov2.vars.farmspeed
    end
    ...
    if farmclosest then
        for i=0,10 do
            ...
            for e, r in next, game.Workspace.Collectibles:GetChildren() do
                ...
                itb = false
                if r:FindFirstChildOfClass("Decal") and macrov2.toggles.enabletokenblacklisting then
                    if api.findvalue(macrov2.bltokens, string.split(r:FindFirstChildOfClass("Decal").Texture, "rbxassetid://")[2]) then
                        itb = true
                    end
                end
    ...
    if place:FindFirstChild("Territory") then
        ...
        while not place:FindFirstChild("TimerLabel", true).Visible and tick() - timestamp < 25 do
            if tick() - secondstamp > 2 then
                api.tween(3, CFrame.new(point + Vector3.new(0, 30, 0)))
                api.humanoidrootpart().Velocity = Vector3.new(0, 0, 0)
                task.wait(1)
                secondstamp = tick()
            end
            task.wait()
            api.tween(3, CFrame.new(point))
            api.humanoidrootpart().Velocity = Vector3.new(0, 0, 0)
        end
...
function killmobs()
    if macrov2.toggles.smartmobkill then
        local monsternames = {
            "Mantis",
            "Scorpion",
            "Spider",
                    ...
                    if decal and decal.Texture then
                        if decal.Texture == "rbxassetid://1629547638" then
                            ...
                            if player.Character:FindFirstChild("Humanoid") and smallest > 20 and smallest < 100 then
                                ...
                                api.tween(1, CFrame.new(token.CFrame.p))
                                task.wait(0.5)
...
function collectplanters()
    ...
    for i, v in pairs(planterst.plantername) do
        if api.partwithnamepart(v, game.Workspace.Planters) and api.partwithnamepart(v, game.Workspace.Planters):FindFirstChild("Soil") then
            ...
            api.tween(3, CFrame.new(soil.CFrame))
    ...
    if temptable.running == false then
        for e, r in next, game.Workspace.Collectibles:GetChildren() do
            if r:FindFirstChildOfClass("Decal") then
                ...
                if aaaaaaaa ~= nil and api.findvalue(macrov2.priority, aaaaaaaa) then
...
function dobubbles()
    if macrov2.toggles.farmpuffshrooms and game.Workspace.Happenings.Puffshrooms:FindFirstChildOfClass("Model") then return end
    ...
    macrov2.vars.walkspeed = macrov2.vars.walkspeed * 1.75
...
function docrosshairs()
    ...
    for _,v in pairs(game.Workspace.Particles:GetChildren()) do
        if string.find(v.Name, "Crosshair") and v.Parent and player.Character and player.Character:FindFirstChild("HumanoidRootPart") and v.BrickColor ~= BrickColor.new("Flint") then
            if macrov2.toggles.fastcrosshairs then
                ...
                if getBuffTime("8172818074") > 0.5 and getBuffStack("8172818074") > 9 and getBuffTime("5101329167") == 0 then
                    if v.BrickColor == BrickColor.new("Alder") then
                        ...
                        repeat
                            task.wait()
                            api.tween(0.1, CFrame.new(v.Position))
...
guiElements["toggles"]["enablestatuspanel"] = information:CreateToggle("Status Panel", true, function(bool)
    macrov2.toggles.enablestatuspanel = bool
    for _,v in pairs(game:GetService("CoreGui"):GetDescendants()) do
...
local convertatslider = farmo:CreateSlider("Convert At", 0, 100, 100, false, function(Value) macrov2.vars.convertat = Value end)
...
guiElements["vars"]["autodigmode"] = farmo:CreateDropdown("Autodig Mode", {"Normal", "Collector Steal"}, function(Option) macrov2.vars.autodigmode = Option end)
...
guiElements["toggles"]["autosprinkler"] = farmo:CreateToggle("Auto Sprinkler", nil, function(State) macrov2.toggles.autosprinkler = State end)
guiElements["toggles"]["farmbubbles"] = farmo:CreateToggle("Farm Bubbles", nil, function(State) macrov2.toggles.farmbubbles = State end)
guiElements["toggles"]["farmflame"] = farmo:CreateToggle("Farm Flames", nil, function(State) macrov2.toggles.farmflame = State end)
guiElements["toggles"]["farmcoco"] = farmo:CreateToggle("Farm Coconuts & Shower", nil, function(State) macrov2.toggles.farmcoco = State end)
guiElements["toggles"]["collectcrosshairs"] = farmo:CreateToggle("Farm Precise Crosshairs", nil, function(State) macrov2.toggles.collectcrosshairs = State end)
guiElements["toggles"]["fastcrosshairs"] = farmo:CreateToggle("Smart Precise Crosshairs ["..Danger.."]", nil, function(State) macrov2.toggles.fastcrosshairs = State end)
guiElements["toggles"]["farmfuzzy"] = farmo:CreateToggle("Farm Fuzzy Bombs", nil, function(State) macrov2.toggles.farmfuzzy = State end)
guiElements["toggles"]["farmunderballoons"] = farmo:CreateToggle("Farm Under Balloons", nil, function(State) macrov2.toggles.farmunderballoons = State end)
guiElements["toggles"]["farmclouds"] = farmo:CreateToggle("Farm Under Clouds", nil, function(State) macrov2.toggles.farmclouds = State end)
farmo:CreateLabel("")
...
farmo:CreateToggle("Farm Closest Leaves", nil, function(State) macrov2.toggles.farmclosestleaf = State end)

local farmt = farmtab:CreateSection("Farming")
guiElements["toggles"]["autodispense"] = farmt:CreateToggle("Auto Dispenser [⚙]", nil, function(State) macrov2.toggles.autodispense = State end)
guiElements["toggles"]["autoboosters"] = farmt:CreateToggle("Auto Field Boosters [⚙]", nil, function(State) macrov2.toggles.autoboosters = State end)
guiElements["toggles"]["clock"] = farmt:CreateToggle("Auto Wealth Clock", nil, function(State) macrov2.toggles.clock = State end)
-- BEESMAS MARKER farmt:CreateToggle("Auto Gingerbread Bears", nil, function(State) macrov2.toggles.collectgingerbreads = State end)
-- BEESMAS MARKER farmt:CreateToggle("Auto Samovar", nil, function(State) macrov2.toggles.autosamovar = State end)
-- BEESMAS MARKER farmt:CreateToggle("Auto Stockings", nil, function(State) macrov2.toggles.autostockings = State end)
-- BEESMAS MARKER farmt:CreateToggle("Auto Honey Candles", nil, function(State) macrov2.toggles.autocandles = State end)
-- BEESMAS MARKER farmt:CreateToggle("Auto Beesmas Feast", nil, function(State) macrov2.toggles.autofeast = State end)
-- BEESMAS MARKER farmt:CreateToggle("Auto Onett's Lid Art", nil, function(State) macrov2.toggles.autoonettart = State end)
guiElements["toggles"]["freeantpass"] = farmt:CreateToggle("Auto Free Antpasses", nil, function(State) macrov2.toggles.freeantpass = State end)
guiElements["toggles"]["farmsprouts"] = farmt:CreateToggle("Farm Sprouts", nil, function(State) macrov2.toggles.farmsprouts = State end)
guiElements["toggles"]["farmpuffshrooms"] = farmt:CreateToggle("Farm Puffshrooms", nil, function(State) macrov2.toggles.farmpuffshrooms = State end)
-- BEESMAS MARKER farmt:CreateToggle("Farm Snowflakes ["..Danger.."]", nil, function(State) macrov2.toggles.farmsnowflakes = State end)
guiElements["toggles"]["farmrares"] = farmt:CreateToggle("Teleport To Rares ["..Danger.."]", nil, function(State) macrov2.toggles.farmrares = State end)
guiElements["toggles"]["autoquest"] = farmt:CreateToggle("Auto Accept/Confirm Quests [⚙]", nil, function(State) macrov2.toggles.autoquest = State end)
guiElements["toggles"]["autodoquest"] = farmt:CreateToggle("Auto Do Quests [⚙]", nil, function(State) macrov2.toggles.autodoquest = State end)
guiElements["toggles"]["honeystorm"] = farmt:CreateToggle("Auto Honeystorm", nil, function(State) macrov2.toggles.honeystorm = State end)
farmt:CreateLabel(" ")
...
guiElements["toggles"]["autoplanters"] = plantersection:CreateToggle("Auto Planters", nil, function(State) macrov2.toggles.autoplanters = State end)
guiElements["toggles"]["blacklistinvigorating"] = plantersection:CreateToggle("Blacklist Invigorating", nil, function(State) macrov2.toggles.blacklistinvigorating = State end)
guiElements["toggles"]["blacklistcomforting"] = plantersection:CreateToggle("Blacklist Comforting", nil, function(State) macrov2.toggles.blacklistcomforting = State end)
guiElements["toggles"]["blacklistmotivating"] = plantersection:CreateToggle("Blacklist Motivating", nil, function(State) macrov2.toggles.blacklistmotivating = State end)
guiElements["toggles"]["blacklistrefreshing"] = plantersection:CreateToggle("Blacklist Refreshing", nil, function(State) macrov2.toggles.blacklistrefreshing = State end)
guiElements["toggles"]["blacklistsatisfying"] = plantersection:CreateToggle("Blacklist Satisfying", nil, function(State) macrov2.toggles.blacklistsatisfying = State end)
...
guiElements["toggles"]["plasticplanter"] = plantersection:CreateToggle("Blacklist Plastic Planter", nil, function(State) macrov2.toggles.plasticplanter = State end)
guiElements["toggles"]["candyplanter"] = plantersection:CreateToggle("Blacklist Candy Planter", nil, function(State) macrov2.toggles.candyplanter = State end)
guiElements["toggles"]["redclayplanter"] = plantersection:CreateToggle("Blacklist Red Clay Planter", nil, function(State) macrov2.toggles.redclayplanter = State end)
guiElements["toggles"]["blueclayplanter"] = plantersection:CreateToggle("Blacklist Blue Clay Planter", nil, function(State) macrov2.toggles.blueclayplanter = State end)
guiElements["toggles"]["tackyplanter"] = plantersection:CreateToggle("Blacklist Tacky Planter", nil, function(State) macrov2.toggles.tackyplanter = State end)
guiElements["toggles"]["pesticideplanter"] = plantersection:CreateToggle("Blacklist Pesticide Planter", nil, function(State) macrov2.toggles.pesticideplanter = State end)
guiElements["toggles"]["petalplanter"] = plantersection:CreateToggle("Blacklist Petal Planter", nil, function(State) macrov2.toggles.petalplanter = State end)
...
guiElements["toggles"]["docustomplanters"] = customplanterssection:CreateToggle("Custom Planters", nil, function(State) macrov2.toggles.docustomplanters = State end)
...
--mobkill:CreateToggle("Train Commando", nil, function(State)
    --macrov2.toggles.traincommando = State
    --if State then
        --api.tween(3, CFrame.new(534.851196, 62.6348724, 165.767883))
    --end
--end)
...
guiElements["toggles"]["killvicious"] = mobkill:CreateToggle("Kill Vicious", nil, function(State) macrov2.toggles.killvicious = State end)
guiElements["toggles"]["killwindy"] = mobkill:CreateToggle("Kill Windy", nil, function(State) macrov2.toggles.killwindy = State end)
local autokillmobstoggle = mobkill:CreateToggle("Auto Kill Mobs", nil, function(State) macrov2.toggles.autokillmobs = State end)
autokillmobstoggle:AddToolTip("Kills mobs after x pollen converting")
guiElements["toggles"]["autokillmobs"] = autokillmobstoggle
guiElements["toggles"]["avoidmobs"] = mobkill:CreateToggle("Avoid Mobs", nil, function(State) macrov2.toggles.avoidmobs = State end)
local autoanttoggle = mobkill:CreateToggle("Auto Ant", nil, function(State) macrov2.toggles.autoant = State end)
...
miscc:CreateButton("Ant Challenge Semi-Godmode", function()
    api.tween(3, CFrame.new(93.4228, 32.3983, 553.128))
    task.wait(1)
    game.ReplicatedStorage.Events.ToyEvent:FireServer("Ant Challenge")
...
raresettings:CreateButton("Add Token To Rares List", function()
    table.insert(macrov2.rares, rarename)
    game:GetService("CoreGui"):FindFirstChild(_G.windowname).Main:FindFirstChild("Rares List Dropdown", true):Destroy()
    raresettings:CreateDropdown("Rares List", macrov2.rares, function(Option) end)
end)
raresettings:CreateButton("Remove Token From Rares List", function()
    table.remove(macrov2.rares, api.tablefind(macrov2.rares, rarename))
...
raresettings:CreateButton("Add Token To Blacklist", function()
    table.insert(macrov2.bltokens, rarename)
    game:GetService("CoreGui"):FindFirstChild(_G.windowname).Main:FindFirstChild(
        "Tokens Blacklist Dropdown", true):Destroy()
    raresettings:CreateDropdown("Tokens Blacklist", macrov2.bltokens,
                                function(Option) end)
end)
raresettings:CreateButton("Remove Token From Blacklist", function()
    table.remove(macrov2.bltokens, api.tablefind(macrov2.bltokens, rarename))
    game:GetService("CoreGui"):FindFirstChild(_G.windowname).Main:FindFirstChild(
        "Tokens Blacklist Dropdown", true):Destroy()
    raresettings:CreateDropdown("Tokens Blacklist", macrov2.bltokens,
                                function(Option) end)
...
fieldsettings:CreateButton("Add Field To Blacklist", function()
    table.insert(macrov2.blacklistedfields, temptable.blackfield)
    game:GetService("CoreGui"):FindFirstChild(_G.windowname).Main:FindFirstChild("Blacklisted Fields Dropdown", true):Destroy()
    fieldsettings:CreateDropdown("Blacklisted Fields", macrov2.blacklistedfields, function(Option) end)
end)
fieldsettings:CreateButton("Remove Field From Blacklist", function()
    table.remove(macrov2.blacklistedfields, api.tablefind(macrov2.blacklistedfields, temptable.blackfield))
    game:GetService("CoreGui"):FindFirstChild(_G.windowname).Main:FindFirstChild("Blacklisted Fields Dropdown", true):Destroy()
    fieldsettings:CreateDropdown("Blacklisted Fields", macrov2.blacklistedfields, function(Option) end)
end)
fieldsettings:CreateDropdown("Blacklisted Fields", macrov2.blacklistedfields, function(Option) end)
...
guiElements["toggles"]["allquests"] = aqs:CreateToggle("Non-Repeatable Quests", nil, function(State) macrov2.toggles.allquests = State end)
guiElements["toggles"]["buckobeequests"] = aqs:CreateToggle("Bucko Bee Quests", nil, function(State) macrov2.toggles.buckobeequests = State end)
guiElements["toggles"]["rileybeequests"] = aqs:CreateToggle("Riley Bee Quests", nil, function(State) macrov2.toggles.rileybeequests = State end)
guiElements["toggles"]["blackbearquests"] = aqs:CreateToggle("Black Bear Quests", nil, function(State) macrov2.toggles.blackbearquests = State end)
guiElements["toggles"]["brownbearquests"] = aqs:CreateToggle("Brown Bear Quests", nil, function(State) macrov2.toggles.brownbearquests = State end)
guiElements["toggles"]["polarbearquests"] = aqs:CreateToggle("Polar Bear Quests", nil, function(State) macrov2.toggles.polarbearquests = State end)
...
guiElements["toggles"]["tptonpc"] = aqs:CreateToggle("Teleport To NPC", nil, function(State) macrov2.toggles.tptonpc = State end)
guiElements["toggles"]["autoquesthoneybee"] = aqs:CreateToggle("Include Honey Bee Quests", nil, function(State) macrov2.toggles.autoquesthoneybee = State end)
guiElements["toggles"]["buyantpass"] = aqs:CreateToggle("Buy Ant Pass When Needed", nil, function(State) macrov2.toggles.buyantpass = State end)
guiElements["toggles"]["smartmobkill"] = aqs:CreateToggle("Modify Mob Kill To Match Quests", nil, function(State) macrov2.toggles.smartmobkill = State end)
guiElements["toggles"]["usegumdropsforquest"] = aqs:CreateToggle("Use Gumdrops For Goo Quests", nil, function(State) macrov2.toggles.usegumdropsforquest = State end)
...
pts:CreateButton("Add Token To Priority List", function()
    table.insert(macrov2.priority, rarename)
    game:GetService("CoreGui"):FindFirstChild(_G.windowname).Main:FindFirstChild("Priority List Dropdown", true):Destroy()
    pts:CreateDropdown("Priority List", macrov2.priority, function(Option) end)
end)
pts:CreateButton("Remove Token From Priority List", function()
    table.remove(macrov2.priority, api.tablefind(macrov2.priority, rarename))
    game:GetService("CoreGui"):FindFirstChild(_G.windowname).Main:FindFirstChild("Priority List Dropdown", true):Destroy()
    pts:CreateDropdown("Priority List", macrov2.priority, function(Option) end)
end)
pts:CreateDropdown("Priority List", macrov2.priority, function(Option) end)
...
game.Workspace.Particles.ChildAdded:Connect(function(v)
    if (v:IsA("Part") or v:IsA("MeshPart")) and not temptable.started.ant and not temptable.started.vicious and macrov2.toggles.autofarm and not temptable.converting and not temptable.planting then
        if v.Name == "WarningDisk" and macrov2.toggles.farmcoco then
            task.wait(0.5)
            if v.BrickColor == BrickColor.new("Lime green") then
                ...
                if temptable.lookat then
                    api.humanoidrootpart().Velocity = Vector3.new(0, 0, 0)
                    api.tween(0.1,CFrame.new(v.CFrame.p, temptable.lookat))
                    task.wait()
                    api.tween(0.1,CFrame.new(v.CFrame.p, temptable.lookat))
                else
                    api.humanoidrootpart().Velocity = Vector3.new(0, 0, 0)
                    api.tween(0.1, CFrame.new(v.CFrame.p))
                    task.wait()
                    api.tween(0.1, CFrame.new(v.CFrame.p))
                end
            end
        ...
        elseif string.find(v.Name, "Bubble") and getBuffTime("5101328809") > 0.2 and macrov2.toggles.farmbubbles then
            if not macrov2.toggles.farmpuffshrooms or (macrov2.toggles.farmpuffshrooms and not game.Workspace.Happenings.Puffshrooms:FindFirstChildOfClass("Model")) then
                if (v.Position - api.humanoidrootpart().Position).magnitude > 100 then return end
...
task.spawn(function()
    while task.wait() do
        ...
        if player.Character:FindFirstChild("ProgressLabel", true) then
            ...
            fieldselected = game.Workspace.FlowerZones[macrov2.vars.field]
            ...
            if macrov2.toggles.autofarm then
                ...
                if macrov2.toggles.autodoquest and player.PlayerGui.ScreenGui.Menus.Children.Quests.Content:FindFirstChild("Frame") then
                    for i, v in next, player.PlayerGui.ScreenGui.Menus.Children.Quests:GetDescendants() do
                        if v.Name == "Description" and v.Parent and v.Parent.Parent then
                            local text = v.Text
                            ...
                            if (macrov2.toggles.buckobeequests and questName:find("Bucko Bee")) or (macrov2.toggles.rileybeequests and questName:find("Riley Bee")) or (macrov2.toggles.polarbearquests and questName:find("Polar Bear")) or (macrov2.toggles.brownbearquests and questName:find("Brown Bear")) or (macrov2.toggles.blackbearquests and questName:find("Black Bear")) or (macrov2.toggles.allquests and not questName:find("Bear:") and not questName:find("Bee:")) then
                                if not string.find(v.Text, "Puffshroom") then
                                    if text:find(" Goo ") then
                                        temptable.usegumdropsforquest = true
                                    end
                                    if api.returnvalue(fieldstable, text) and not string.find(v.Text, "Complete!") and not api.findvalue(macrov2.blacklistedfields, api.returnvalue(fieldstable, text)) then
                                    ...
                                    elseif api.returnvalue(pollentypes, text) and not string.find(v.Text, "Complete!") then
                                        ...
                                        if d == "Blue Flowers" or d == "Blue Pollen" then
                                            fieldselected = game.Workspace.FlowerZones[macrov2.bestfields.blue]
                                            break
                                        elseif d == "White Flowers" or d == "White Pollen" then
                                            fieldselected = game.Workspace.FlowerZones[macrov2.bestfields.white]
                                            break
                                        elseif d == "Red Flowers" or d == "Red Pollen" then
                                            fieldselected = game.Workspace.FlowerZones[macrov2.bestfields.red]
                                            break
                                        end
                                    ...
                                    elseif string.find(text, "Ants.") and not string.find(text, "Complete!") then
                                        if rtsg().Eggs.AntPass == 0 and macrov2.toggles.buyantpass then
                                        ...
                                        if not game.Workspace.Toys["Ant Challenge"].Busy.Value and rtsg().Eggs.AntPass > 0 then
                                            if macrov2.vars.questcolorprefer == "Any NPC" then
                                                farmant()
                                            else
                                                if questName:find(macrov2.vars.questcolorprefer) then
                                                    farmant()
                                                end
                                            end
                                        end
                        ...
                        if i == #player.PlayerGui.ScreenGui.Menus.Children.Quests:GetDescendants() then
                            if macrov2.toggles.followplayer then
                                local playerToFollow = game.Players:FindFirstChild(macrov2.vars.playertofollow)
                ...
                if temptable.sprouts.detected and temptable.sprouts.coords and not temptable.started.vicious and macrov2.toggles.farmsprouts then
                ...
                if macrov2.toggles.convertballoons and not temptable.planting and not temptable.started.vicious and macrov2.vars.convertballoonpercent and gethiveballoon() and getBuffTime("8083443467") < tonumber(macrov2.vars.convertballoonpercent) / 100 then
                    ...
                    task.wait(6)
                    if macrov2.toggles.autoant and not game.Workspace.Toys["Ant Challenge"].Busy.Value and rtsg().Eggs.AntPass > 0 then
                        farmant()
                    end
                ...
                elseif tonumber(temptable.pollenpercentage) >= tonumber(macrov2.vars.convertat) and not temptable.started.vicious and not temptable.planting then
...
task.spawn(function()
    while task.wait(1) do
        if macrov2.toggles.killvicious and temptable.detected.vicious and not temptable.converting and not temptable.started.monsters and not game.Workspace.Toys["Ant Challenge"].Busy.Value then
            ...
            for i, v in next, game.workspace.Particles:GetChildren() do
                for x in string.gmatch(v.Name, "Vicious") do
                    if string.find(v.Name, "Vicious") then
                        api.tween(3, CFrame.new(v.Position.x, v.Position.y, v.Position.z))
                        task.wait(1)
                        api.tween(3, CFrame.new(v.Position.x, v.Position.y, v.Position.z))
                        task.wait(.5)
                    end
...
task.spawn(function()
    while task.wait() do
        if macrov2.toggles.killwindy and temptable.detected.windy and not temptable.converting and not temptable.started.vicious and not temptable.started.mondo and not temptable.started.monsters and not game.Workspace.Toys["Ant Challenge"].Busy.Value then
...
task.spawn(function()
    while task.wait() do
        if macrov2.toggles.traincrab and api.humanoidrootpart() then
            api.tween(3, CFrame.new(-307.52117919922, 110.11863250732, 467.86791992188))
        end
...
task.spawn(function()
    while task.wait(0.1) do
        if not temptable.converting then
            if macrov2.toggles.autosamovar then
            ...
            if macrov2.toggles.autostockings then
            ...
            if macrov2.toggles.autoonettart then
            ...
            if macrov2.toggles.autocandles then
            ...
            if macrov2.toggles.autofeast then
                game:GetService("ReplicatedStorage").Events.ToyEvent:FireServer(
                    "Beesmas Feast")
...
task.spawn(function()
    while task.wait(2) do
        ...
        if macrov2.toggles.honeystorm and canToyBeUsed("Honeystorm") then
            game.ReplicatedStorage.Events.ToyEvent:FireServer("Honeystorm")
        end
        if macrov2.toggles.collectgingerbreads then
            game:GetService("ReplicatedStorage").Events.ToyEvent:FireServer("Gingerbread House")
        end
        if macrov2.toggles.autodonate then
            if isWindshrineOnCooldown() == false then
                donateToShrine(macrov2.vars.donoItem, macrov2.vars.donoAmount)
            end
        end
        if macrov2.toggles.autodispense and canToyBeUsed("Free Royal Jelly Dispenser") then
            if macrov2.dispensesettings.rj then
                game:GetService("ReplicatedStorage").Events.ToyEvent:FireServer("Free Royal Jelly Dispenser")
            end
            if macrov2.dispensesettings.blub and canToyBeUsed("Blueberry Dispenser") then
                game:GetService("ReplicatedStorage").Events.ToyEvent:FireServer("Blueberry Dispenser")
            end
            if macrov2.dispensesettings.straw and canToyBeUsed("Strawberry Dispenser") then
                game:GetService("ReplicatedStorage").Events.ToyEvent:FireServer("Strawberry Dispenser")
            end
            if macrov2.dispensesettings.treat and canToyBeUsed("Treat Dispenser") then
                game:GetService("ReplicatedStorage").Events.ToyEvent:FireServer("Treat Dispenser")
            end
            if macrov2.dispensesettings.coconut and canToyBeUsed("Coconut Dispenser") then
                game:GetService("ReplicatedStorage").Events.ToyEvent:FireServer("Coconut Dispenser")
            end
            if macrov2.dispensesettings.glue and canToyBeUsed("Glue Dispenser") then
                game:GetService("ReplicatedStorage").Events.ToyEvent:FireServer("Glue Dispenser")
            end
        end
        if macrov2.toggles.autoboosters then
            if macrov2.dispensesettings.white and canToyBeUsed("Field Booster") then
                game.ReplicatedStorage.Events.ToyEvent:FireServer("Field Booster")
            end
            if macrov2.dispensesettings.red and canToyBeUsed("Red Field Booster") then
                game.ReplicatedStorage.Events.ToyEvent:FireServer("Red Field Booster")
            end
            if macrov2.dispensesettings.blue and canToyBeUsed("Blue Field Booster") then
                game.ReplicatedStorage.Events.ToyEvent:FireServer("Blue Field Booster")
            end
        end
        if macrov2.toggles.clock and canToyBeUsed("Wealth Clock") then
            game:GetService("ReplicatedStorage").Events.ToyEvent:FireServer("Wealth Clock")
        end
        if macrov2.toggles.freeantpass and canToyBeUsed("Free Ant Pass Dispenser") then
        ...
        if macrov2.toggles.autoquest then
            ...
            if macrov2.toggles.autoquesthoneybee then
...
game:GetService("RunService").Heartbeat:connect(function()
    if macrov2.toggles.autoquest and player:FindFirstChild("PlayerGui") and player.PlayerGui:FindFirstChild("ScreenGui") and player.PlayerGui.ScreenGui:FindFirstChild("NPC") and player.PlayerGui.ScreenGui.NPC.Visible then
        firesignal(player.PlayerGui.ScreenGui.NPC.ButtonOverlay.MouseButton1Click)
    end
    if macrov2.toggles.loopspeed and player.Character:FindFirstChild("Humanoid") then
        player.Character.Humanoid.WalkSpeed = macrov2.vars.walkspeed
    end
    if macrov2.toggles.loopjump and player.Character:FindFirstChild("Humanoid") then
        player.Character.Humanoid.JumpPower = macrov2.vars.jumppower
    end
end)
...
task.spawn(function()
    while task.wait() do
        if macrov2.toggles.farmsnowflakes then
            task.wait(3)
...
task.spawn(function()
    while task.wait() do
        ...
        if not temptable.started.ant and not temptable.started.vicious and macrov2.toggles.autofarm and not temptable.converting then
            if torso then
...
task.spawn(function()
    while task.wait(5) do
        if macrov2.toggles.docustomplanters then
            ...
            for i=1,3 do
                if not isfile("macrov2/plantercache/cycle"..i.."cache.file") then
                    for _,planter in pairs(fetchAllPlanters()) do
                        RequestCollectPlanter(planter)
                    end
                    writefile("macrov2/plantercache/cycle"..i.."cache.file", "1")
                end
            end
            ...
            if not temptable.started.ant and macrov2.toggles.autofarm and not temptable.converting and not temptable.started.monsters then
                for i,cycle in pairs(plantercycles) do
                    ...
                    local planted = false
                    local currentstep = isfile("macrov2/plantercache/cycle"..i.."cache.file") and tonumber(readfile("macrov2/plantercache/cycle"..i.."cache.file")) or 1
                    ...
                    if not planted and cycle[currentstep].Planter and #fetchAllPlanters() < 3 then
                        ...
                        if planter == "The Planter Of Plenty" and GetItemListWithValue()["PlentyPlanter"] and GetItemListWithValue()["PlentyPlanter"] > 0 then
                            ...
                            writefile("macrov2/plantercache/cycle"..i.."cache.file", tostring((currentstep - 1) % steps[i] + 2))
                        else
...
task.spawn(function()
    pcall(function()
        ...
        while task.wait(1) do
            if macrov2.toggles.enablestatuspanel then
...
if _G.autoload then
    if isfile("macrov2/BSS_" .. _G.autoload .. ".json") then
        macrov2 = game:service("HttpService"):JSONDecode(readfile("macrov2/BSS_" .. _G.autoload .. ".json"))
        for i,v in pairs(guiElements) do
            for j,k in pairs(v) do
                local obj = k:GetObject()
                local lastCharacters = obj.Name:reverse():sub(0, obj.Name:reverse():find(" ")):reverse()
...
for _, part in next, workspace:FindFirstChild("FieldDecos"):GetDescendants() do
    if part:IsA("BasePart") then
        ...
        part.Transparency = part.Transparency < 0 and 0 or part.Transparency
        task.wait()
    end
end
